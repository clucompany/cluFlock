...
impl<T> WaitFlockUnlockRange for T where T: FlockElement<FilePtr = RawFilePtr> {
	#[inline]
	...
	unsafe fn unlock_range<NS, NE, C>(&mut self, range: impl Into<FlockRange<NS, NE, C>>) -> Result<(), IoError> where NS: FlockRangePNumBeh, NE: FlockRangePNumBeh + PartialOrd<NS>, C: FlockRangePNumBehChecker {
		WaitFlockUnlockRange::unlock_range_fn(
			self,
			range,
...
fn next_force_flock_ignore_result<
	...
	NS: FlockRangePNumBeh, NE: FlockRangePNumBeh + PartialOrd<NS>, C: FlockRangePNumBehChecker,
	
>(data: FE, flag: FLM::InFlags, range: IRange) {
...
fn next_force_flock<
	...
	R
>(data: FE, flag: FLM::InFlags, range: IRange, next: N, errf: NF) -> R {
	let range = range.into();

...
/// Determination of the logic of the range operation.
pub trait FlockRangePNumBeh:
	Sized + Debug + Clone + Copy + PartialEq + PartialOrd + Eq + Ord + Hash
{
	...
	/// scheme: end >= *self
	#[inline(always)]
	fn is_valid_endrange<NE: FlockRangePNumBeh + PartialOrd<Self>>(&self, end: NE) -> bool {
		if Self::IS_IGNORE_NUM || Self::IS_ONEPOS || Self::IS_FULLRANGE {
			panic!("Undefined behavior detected.");
		}
...
pub(crate) use __make_auto_pnum_type;
...
fn test_flock_rangepnum_valid() {
	...
	__make_auto_pnum_type!(__TestNum);

	assert!(!__TestNum(-1).is_valid());
	assert!(__TestNum(0).is_valid());
	assert!(__TestNum(1).is_valid());
	assert!(__TestNum(2).is_valid());
	assert!(!__TestNum(3).is_valid());
}

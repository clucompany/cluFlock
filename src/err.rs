...
impl<T> FlockError<T> where T: FlockElement {
	...
	#[inline(always)]
	pub const fn get_debug_data(&self) -> &impl Debug where T: Debug {
		&self.data
	}
	...
	/// Get a link to err.
	#[inline(always)]
	pub /*const*/ fn as_mut_err(&mut self) -> &mut IoError {
		&mut self.err
	}
	...
	#[inline(always)]
	pub /*const*/ fn into_all(self) -> (T, IoError) {
		(self.data, self.err)
	}
	
	/// Get only the error from the error structure.
	#[inline(always)]
	pub /*const*/ fn into_err(self) -> IoError {
		self.err
	}
}
...
impl<T> DerefMut for FlockError<T> where T: FlockElement {
	#[inline(always)]

...
__make_auto_pnum_type!(FlockRangePNum);
...
impl<T> TryFlockUnlock for T
where
	T: FlockElement<FilePtr = RawFilePtr>,
{
	#[inline]
	unsafe fn unlock_no_result(&mut self) {
		next_force_flock_ignore_result(self, __internal_flags::TRY_UNLOCK)
	}

	#[inline]
	unsafe fn unlock(&mut self) -> Result<(), IoError> {
		TryFlockUnlock::unlock_fn(self, || Ok(()), Err)
	}
	...
	T: FlockElement<FilePtr = RawFilePtr>,
{
	#[inline]
	unsafe fn unlock_no_result(&mut self) {
		next_force_flock_ignore_result(self, __internal_flags::WAIT_UNLOCK)
	}

	#[inline]
	unsafe fn unlock(&mut self) -> Result<(), IoError> {
		WaitFlockUnlock::unlock_fn(self, || Ok(()), Err)
	}
	...
	T: FlockElement<FilePtr = RawFilePtr>,
{
	...
	fn try_lock_fn<R>(
		...
		next_safe_flock(self, __internal_flags::TRY_SHARED_LOCK, next, errf)
	}
	...
	fn wait_lock_fn<R>(
		...
		next_safe_flock(self, __internal_flags::WAIT_SHARED_LOCK, next, errf)
	}
}
	...
	T: FlockElement<FilePtr = RawFilePtr>,
{
	...
	fn try_lock_fn<R>(
		...
		next_safe_flock(self, __internal_flags::TRY_EXCLUSIVE_LOCK, next, errf)
	}
	...
	fn wait_lock_fn<R>(
		...
		next_safe_flock(self, __internal_flags::WAIT_EXCLUSIVE_LOCK, next, errf)
	}
}

#[inline(always)]
fn next_force_flock_ignore_result<D: FlockElement<FilePtr = RawFilePtr>>(
	data: D,
	flag: __internal_flags::LibcFlag,
) {
	unsafe {
...
fn next_force_flock<D: FlockElement<FilePtr = RawFilePtr>, R>(
	data: D,
	flag: __internal_flags::LibcFlag,
	next: impl FnOnce(D) -> R,
	errf: impl FnOnce(FlockError<D>) -> R,
) -> R {
	let result = unsafe {
		let ptr = FlockElement::as_file_ptr(&data);
...
fn next_safe_flock<D: FlockElement<FilePtr = RawFilePtr>, R>(
	data: D,
	flag: __internal_flags::LibcFlag,
	next: impl FnOnce(FlockLock<D>) -> R,
	errf: impl FnOnce(FlockError<D>) -> R,
) -> R {
	next_force_flock(
		data,

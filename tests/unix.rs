...
mod unix {
	...
	impl FlockProcess {
		...
		pub fn sleep_exclusive(path: &Path, i: usize) -> Self {
			Self::new(
				...
				"-x",
				&[
					OsStr::new("sleep").into(),
					OsString::from(i.to_string()).into(),
				] as &[Cow<'static, OsStr>],
			)
		}
		#[allow(dead_code)]
	...
	fn unix_exclusive_check() {
		...
		if let Ok(a) = ExclusiveFlock::try_lock(&*file) {
			panic!(
				"Strange behavior, we 've already made a lock in another process.., {:?}",
				a
			);
		}
	...
	fn unix_shared_check() {
		...
		if let Err(a) = SharedFlock::try_lock(&*file) {
			//two shared, process wait shared!
			panic!(
				"Strange behavior, we 've already made a lock in another process.., {:?}",
				a
			);
